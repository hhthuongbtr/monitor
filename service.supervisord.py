...
def create_supervisord_config(ip, name):
    file = File()
...
def callback(ch, method, properties, body):
    ...
    if not is_ip(body):
        logger.debug("received " + body + "not ip.")
        try:
            spvs_code = int(body)
            logger.debug("received " + body + "is Supervisord state code")
            if spvs_code == 100:
    ...
    name = name.replace(" ", "")
    create_supervisord_config(ip, name)
